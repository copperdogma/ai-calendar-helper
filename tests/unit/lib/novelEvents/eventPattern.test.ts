...
describe("EventPattern", () => {
  ...
  it("rejects different calendar", () => {
    expect(basePattern.isSimilarTo(makeEvent({ calendarId: "personal" }))).toBe(
      false,
    );
  });
  ...
  it("rejects event more than 1h away", () => {
    expect(
      basePattern.isSimilarTo(
        makeEvent({ start: new Date("2025-01-06T11:30:00Z") }),
      ),
    ).toBe(false);
  });
  ...
  it("allows within 60 minute window", () => {
    expect(
      basePattern.isSimilarTo(
        makeEvent({ start: new Date("2025-01-06T09:55:00Z") }),
      ),
    ).toBe(true);
  });
  ...
  it("requires exact title when low frequency", () => {
    ...
    expect(lowFreqPattern.isSimilarTo(makeEvent({ title: "Team Mtg" }))).toBe(
      false,
    );
  });
  ...
  it("allows partial match when high frequency and long titles", () => {
    ...
    expect(highFreqPattern.isSimilarTo(makeEvent({ title: "Team Meeting Q1" }))).toBe(
      true,
    );
