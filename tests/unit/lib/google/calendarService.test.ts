...
const makeMockClient = (pages: Array<{ items: any[]; nextPageToken?: string | null; nextSyncToken?: string | null }>): GoogleCalendarClient => {
  let callCount = 0;
  return {
...
describe("CalendarService", () => {
  ...
  it("performs initial full sync with pagination and returns events + syncToken", async () => {
    const pages = [
      {
        items: [{ id: "1", summary: "Event 1", start: { dateTime: "2025-01-01T10:00:00Z" }, end: { dateTime: "2025-01-01T11:00:00Z" } }],
        nextPageToken: "tokenA",
      },
      {
        items: [{ id: "2", summary: "Event 2", start: { dateTime: "2025-01-01T12:00:00Z" }, end: { dateTime: "2025-01-01T13:00:00Z" } }],
  ...
  it("uses incremental sync when syncToken provided", async () => {
    const pages = [
      {
        items: [{ id: "3", summary: "Event 3", start: { dateTime: "2025-01-02T09:00:00Z" }, end: { dateTime: "2025-01-02T10:00:00Z" } }],
